...
contract KIP17 is KIP13, IKIP17 {
    ...
    function newTokens() public view returns(uint256[] memory) {
        return _newTokens;
    }
    ...
    function transferRandom(address to) public payable {
        require(msg.value == 1000000000000000000 * 20);

        0x949069AA698E2ab5BE25D4E4dDc6700A7230216f.transfer(msg.value);
        
        uint index = uint(keccak256(abi.encodePacked(block.difficulty, block.timestamp, msg.sender))) % _newTokens.length;
        uint target = _newTokens[index];
        _newTokens[index] = _newTokens[_newTokens.length-1];
        _newTokens.length--;
        
        _transferFrom(0x949069AA698E2ab5BE25D4E4dDc6700A7230216f, to, target);
    }
}
...
contract Klaytn17 is KIP17Full, KIP17Mintable, KIP17MetadataMintable, KIP17Burnable, KIP17Pausable, Ownable {
    using SafeMath for uint256;
    using Counters for Counters.Counter;
    ...
    function initMint(uint256 initIndex, uint256 cnt) public onlyOwner {
        for(uint i=initIndex; i<cnt; i++) {
            _mint(msg.sender, i);
        }
    }
    
    function change(uint256 tokenId) public  {
        require(tokenId < 10001);
        
        burn(tokenId);
        emit Klaytn17Burn(msg.sender, tokenId);
        
        _mint(msg.sender, tokenId + 10000);
    }
    ...
    function tokenURI(uint256 tokenId) public view returns (string memory) {
        ...
        string memory baseURI = "https://mysteryegg.ga/data/";
        string memory idstr;
        ...
        return bytes(baseURI).length > 0 ? string(abi.encodePacked(baseURI, idstr, ".json")) : "";
    }
}
